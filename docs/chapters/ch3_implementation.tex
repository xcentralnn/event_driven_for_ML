...
Thay vì phụ thuộc hoàn toàn vào cơ chế mặc định của Knative, chúng tôi xây dựng thêm một lớp kiến trúc vi dịch vụ (Microservices) chạy song song trên Kubernetes để hỗ trợ Lập lịch dự đoán (Predictive Scheduling). Hệ thống bao gồm 3 thành phần chính:

\begin{itemize}
    \item \textbf{Mô hình Dự báo (ML Forecasting Service):} Đây là một service riêng biệt, làm nhiệm vụ phân tích dữ liệu lịch sử để tính toán ra số lượng Pod cần chuẩn bị trước.
    \item \textbf{Bộ điều khiển (Predictive Scaler Controller):} Nhận số liệu từ ML Service, sau đó gọi trực tiếp vào API của Kubernetes để sửa đổi số lượng bản sao (replicas) của Deployment tương ứng.
    \item \textbf{Bảng điều khiển Giám sát (Executive Dashboard):} Giao diện hiển thị trực tiếp các thông số kỹ thuật, số lượng Pod hiện tại và trạng thái dự đoán để dễ theo dõi.
\end{itemize}

Để các thành phần này có dữ liệu hoạt động, chúng tôi sử dụng \textbf{Prometheus}. Prometheus chạy ngầm và liên tục kéo (pull) các số liệu lưu lượng (như Request per second) từ Knative. ML Service sẽ truy vấn vào Prometheus để lấy dữ liệu làm đầu vào phân tích.
